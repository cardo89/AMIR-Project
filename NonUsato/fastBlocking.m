function [ Xi, NB] = fastBlocking(X,height,width)
...
% checking if the dimension is a multiple of heught and widt
X = [X ; zeros(height-mod(SX(1),height),SX(2))];
SX = size(X);
X = [X zeros(SX(1), width-mod(SX(2),width))];
